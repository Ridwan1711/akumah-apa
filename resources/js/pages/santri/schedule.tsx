...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Jadwal Santri', href: '/santri/schedule' },
];

export default function SantriSchedule({ class: schoolClass, semester, week }: Props) {
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Jadwal Santri" />

            <div className="flex h-full flex-1 flex-col gap-4 p-4 md:p-8">
                <div>
                    <h1 className="text-2xl font-semibold text-foreground">Jadwal Santri</h1>
                    <p className="text-sm text-muted-foreground">
                        Kelas {schoolClass.name}{semester ? ` - ${semester.name}` : ''}.
                    </p>
                </div>
