...
export default function ReportCardIndex({ classes, semesters, students, filters }: Props) {
    const [classId, setClassId] = useState(filters.class_id ?? '');
    const [semesterId, setSemesterId] = useState(filters.semester_id ?? '');

    function loadStudents() {
        if (classId && semesterId) {
            router.get('/admin/report-cards', { class_id: classId, semester_id: semesterId }, { preserveState: true });
        }
    }

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Raport Diniyah" />
            <div className="flex h-full flex-1 flex-col gap-4 p-4">
                <Heading title="Raport Diniyah" description="Generate dan cetak raport santri per semester" />
                <FlashMessage />
